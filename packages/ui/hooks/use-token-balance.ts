...
export function useTokenBalance(chain: Chain, debounceMs = 250) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const balance = await fetchTokenBalance({ chain, owner: address })
  ...
  }, [chain, isConnected, address, debounceMs])
