...
export async function fetchTokenBalance(params: BalanceParams): Promise<string> {
  const { chain, owner } = params
  const usdc = TOKENS[chain.key].USDC
  if (!usdc) throw new Error(`Could not find USDC address for chain ${chain.name}`)

  const calldata = encodeBalanceOf(owner)
  const response = await fetch('/api/evm-call', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chainId: chain.id, address: usdc.address, data: calldata }),
  })
  ...
  return toStringDecimal(balance, usdc.decimals)
}
